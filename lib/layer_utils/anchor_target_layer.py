...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, _feat_stride, all_anchors, num_anchors):
  ...
  # 求每一行的最大值下标，应该就是每个anchor对应分数最大的gt_box
  argmax_overlaps = overlaps.argmax(axis=1)
  # 将每行得到的最大值保存
  max_overlaps = overlaps[np.arange(len(inds_inside)), argmax_overlaps]
  # 求每一列的最大值下标，应该就是每个gt_box对应分数最大的anchor
  gt_argmax_overlaps = overlaps.argmax(axis=0)
  # 同理，保存每列的最大值
  gt_max_overlaps = overlaps[gt_argmax_overlaps,
                             np.arange(overlaps.shape[1])]      # overlaps.shape[1]是gt_box个数
  # 保留那些相等的索引
  ...
  # 前景标签，对于gt_boxes,和anchors重合率最大的检测框标记为1
  labels[gt_argmax_overlaps] = 1

  # fg label: above threshold IOU
  # 将anchor最大的分并且大于RPN_POSITIVE_OVERLAP的标记为1
  ...
  bbox_targets = _compute_targets(anchors, gt_boxes[argmax_overlaps, :])      # return bbox_transform(ex_rois, gt_rois[:, :4]).astype(np.float32, copy=False)
  ...
  if cfg.TRAIN.RPN_POSITIVE_WEIGHT < 0:# __C.TRAIN.RPN_POSITIVE_WEIGHT = -1.0 设置-1使用统一的权重
